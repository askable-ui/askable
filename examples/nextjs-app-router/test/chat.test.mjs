...
const { POST: askablePOST } = await import('../app/api/askable-chat/route.ts');
...
test('Askable endpoint preserves plain text, history order, and one current question', async () => {
  const response = await askablePOST(new Request('http://example.test/api/askable-chat', {
    method: 'POST',
    body: JSON.stringify({
      question: 'And now?',
      context: 'Churn: 2.1%',
      messages: [
        { role: 'user', content: 'What is focused?' },
  ...
  assert.equal(await response.text(), 'Hello dashboard');
  assert.match(systemOf(requests[0]), /Churn: 2.1%/);
  assert.deepEqual(conversationOf(requests[0]), [
    { role: 'user', text: 'What is focused?' },
    { role: 'assistant', text: 'Revenue' },
    { role: 'user', text: 'And now?' },
  ]);
});

test('Askable endpoint accepts a single question without history', async () => {
  const response = await askablePOST(new Request('http://example.test/api/askable-chat', {
    method: 'POST', body: JSON.stringify({ question: 'Hello', context: '' }),
  }));
  assert.equal(await response.text(), 'Hello dashboard');
  assert.deepEqual(conversationOf(requests[0]), [{ role: 'user', text: 'Hello' }]);
});
