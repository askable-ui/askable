...
export interface UseAskablePerformanceSourceOptions
  ...
  id?: string;
  /**
   * Automatically collect navigation timing on mount.
   * @default true
   */
  autoCollect?: boolean;
}
...
export interface UseAskablePerformanceSourceResult extends UseAskableSourceResult {
  ...
  /** Add or update a named metric (e.g. from web-vitals library). */
...
/**
 * React hook that collects Core Web Vitals and navigation timing and exposes
 ...
 * // AI: "Your LCP is 3.8s (rated poor). Consider lazy-loading above-the-fold images."
 * ```
 */
export function useAskablePerformanceSource(
  ...
  const [snapshot, setSnapshot] = useState<AskablePerformanceSourceSnapshot | null>(null);
  const snapshotRef = useRef(snapshot);
  snapshotRef.current = snapshot;

  const source = useMemo(
    () => createAskablePerformanceSource({ describe, kind, getSnapshot: () => snapshotRef.current }),
  ...
  const addMetric = (metricName: string, value: number, rating?: AskablePerformanceMetric['rating']) => {
    ...
    setSnapshot((prev) => {
      ...
      const metrics = [...filtered, entry];
      const hasPoorMetrics = metrics.some((m) => m.rating === 'poor');
      const hasWarningMetrics = metrics.some((m) => m.rating === 'needs-improvement');
      return { navigation: prev?.navigation ?? { loadTime: null, ttfb: null, domContentLoaded: null, dnsTime: null, connectTime: null }, metrics, hasPoorMetrics, hasWarningMetrics, memory: prev?.memory ?? null };
    });
    notifyRef.current();
  };
  ...
  useEffect(() => {
    ...
    const collect = () => {
      ...
      setSnapshot((prev) => {
        ...
        const hasPoorMetrics = metrics.some((m) => m.rating === 'poor');
        const hasWarningMetrics = metrics.some((m) => m.rating === 'needs-improvement');
        return { navigation: nav, metrics, hasPoorMetrics, hasWarningMetrics, memory };
      });
      notifyRef.current();
    };
