...
export function useAskableViewport(options?: UseAskableViewportOptions): UseAskableViewportResult {
  const [visibleItems, setVisibleItems] = useState<AskableFocus[]>([]);
  ...
  useEffect(() => {
    ...
    const flush = () => {
      ...
      const items = Array.from(visible.values()).filter(
  ...
  const promptContext = useMemo(() => {
    if (!visibleItems.length) return 'No annotated elements are currently visible in the viewport.';
