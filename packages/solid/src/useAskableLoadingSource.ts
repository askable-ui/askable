...
/**
 * SolidJS primitive that tracks named loading operations and exposes them to AI
 ...
 * start('users');
 * fetchUsers().then(() => finish('users')).catch((e) => error('users', e.message));
 * ```
 */
export function useAskableLoadingSource(
  ...
  const [snapshot, setSnapshot] = createSignal<AskableLoadingSourceSnapshot | null>(null);

  const source = createAskableLoadingSource({ describe, kind, getSnapshot: snapshot });
  const result = useAskableSource(id, source, { enabled, ctx, name, events });

  function update(): void {
    setSnapshot(buildSnapshot(ops));
