...
/**
 * SolidJS primitive that queries browser permission states and exposes them to
 * AI assistants.
 *
 * @example
 * ```tsx
...
export function useAskablePermissionSource(
  ...
  const [snapshot, setSnapshot] = createSignal<AskablePermissionSourceSnapshot | null>(null);
  ...
  const refresh = async () => {
    ...
    setSnapshot(toSnapshot(entries));
