...
import { useAskable, type UseAskableOptions } from './useAskable.svelte.js';
...
export interface UseAskableKeyboardShortcut {
...
/**
 * Svelte 5 runes-based composable that listens for a keyboard shortcut and fires
 * with the full composed AI context — ready to send to any LLM. Defaults to Cmd+K / Ctrl+K.
 *
 * @example
 * ```svelte
 * <script lang="ts">
 ...
 *
 *   const kc = useAskableKeyboardShortcut({
 *     toggle: true,
...
export function useAskableKeyboardShortcut(
  ...
  let lastContext = $state<string | null>(null);
